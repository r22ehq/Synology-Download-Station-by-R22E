...
test.describe('Service Worker Lifecycle', () => {
  test('reconstructs state on wakeup without duplicates', async ({ page, gotoOptions, gotoPopup, mockNas, extensionId }) => {
    mockNas.state.authStatus = 'SUCCESS';
    mockNas.state.tasks = [{ id: 'task1', title: 'Lifecycle Task', status: 'downloading', size: 100, downloaded: 50 }];
